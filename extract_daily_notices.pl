...
# =============================================================================
# Shoutbomb Daily Notices Extract Script
# Author: Ian Skelskey
# Copyright (C) 2025 Bibliomation Inc.
#
# This script extracts courtesy notice data from Evergreen ILS 
# and sends it to Shoutbomb for SMS notifications.
#
...
logheader("Shoutbomb Daily Notices Extract\nExtracting Courtesy Notices");
...
try {
    ...
    ###########################
    # 5) Extract Courtesy Notices
    ###########################
    logmsg("INFO", "Extracting courtesy notices...");
    my ($courtesy_columns, $courtesy_data) = extract_data(
        $dbh, 
        "$FindBin::Bin/sql/courtesy_notice.sql",
        { org_units => $org_units }
    );
    logmsg("INFO", "Extracted " . scalar(@$courtesy_data) . " courtesy notices");
    ...
    # Write courtesy notices to file
    my $courtesy_file = write_data_to_file(
        "${prefix}_courtesy_${date_str}", 
        $courtesy_data, 
        $courtesy_columns, 
        $conf->{tempdir}
    );
    ...
    # Archive courtesy notices
    my $courtesy_archive = File::Spec->catfile($archive_dir, basename($courtesy_file));
    if (copy($courtesy_file, $courtesy_archive)) {
        logmsg("INFO", "Archived courtesy notices to $courtesy_archive");
        push @archived_files, $courtesy_archive;
    } else {
        logmsg("WARNING", "Failed to archive courtesy notices: $!");
    }
    ...
    if (!$dry_run) {
        ...
        # Use the direct path for courtesy notices (/Renew/)
        my $remote_path = '/Renew';
        
        # Upload courtesy notices using SFTP
        ...
        if ($sftp_error) {
            logmsg("ERROR", "SFTP upload of courtesy notices failed: $sftp_error");
        } else {
            logmsg("INFO", "SFTP upload of courtesy notices successful to $remote_path");
        }
    } else {
    ...
    if (!$dry_run || $sftp_error) {
        ...
        my $subject = $sftp_error 
            ? "ERROR: Shoutbomb Daily Notices Extract Failed" 
            : "SUCCESS: Shoutbomb Daily Notices Extract Completed";
        
        my $body = "<h2>Shoutbomb Daily Notices Extract</h2>\n";
        ...
        $body .= "<li>Courtesy Notices: " . scalar(@$courtesy_data) . "</li>\n";
        $body .= "</ul>\n";
    ...
    ###########################
    # Update last run time for courtesy notices - only if not in dry-run mode
    if (!$dry_run) {
        set_last_run_time($dbh, $org_units, 'courtesy_notice');
    } else {
    ...
    # Clean up files
    if ($conf->{cleanup}) {
        # Clean up temporary files specific to courtesy notices
        cleanup_temp_directory($conf->{tempdir}, 'courtesy');
        
        if (!$dry_run) {
            # Clean up archive files, keeping only the most recent for courtesy notices
            cleanup_archive_files($conf->{archive}, $prefix, 'courtesy');
        }
    ...
    logheader("Finished Shoutbomb Daily Notices Extract\nElapsed time: $formatted_time");
} catch {
    my $error = $_;
    ...
    if (!$dry_run) {
        ...
        my $subject = "ERROR: Shoutbomb Daily Notices Extract Failed";
        my $body = "<h2>Shoutbomb Daily Notices Extract Error</h2>\n";
