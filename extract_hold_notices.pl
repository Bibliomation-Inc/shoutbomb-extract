...
try {
    ...
    if (!$dry_run) {
        ...
        # Use the direct path for hold notices (/Holds/)
        my $remote_path = '/Holds';
        
        # Upload hold notices using SFTP
        ...
        if ($sftp_error) {
            ...
            logmsg("INFO", "SFTP upload of hold notices successful to $remote_path");
        }
    } else {
